...
class FamilyTree(Member):
    ...
    def get_siblings(self, member_name):
        ...
        for sibling in family_tree.values():
            ...
            if (member_father is not None and sibling.get_father() == member_father) or \
               (member_mother is not None and sibling.get_mother() == member_mother):
...
#Otto's side (husband) (Nazar)
    ...
    "volodymyr levchuk": Member("Volodymyr Levchuk", birth_date = "30/11/1967", death_year = None, father = "Yaroslav Levchuk", mother = "Lubov Levchuk"),
